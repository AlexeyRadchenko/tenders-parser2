...
class HttpWorker:
    ...
    @classmethod
    @retry(logger)
    def get_tenders_list(cls, target_param=None, page=None):
        if not target_param:
            res = requests.get(config.tenders_list_url.format(page), proxies=config.proxy)
        else:
            res = requests.post(config.tenders_list_url, data=target_param, proxies=config.proxy)
        return res
